...
class MatrixAccessController(Sofa.Core.Controller):
    ...
    def onAnimateEndEvent(self, event):
        ...
        #forcesNorm = self.target2.getData("constraintForces")
        ...
        #GeomagicDevice.maxInputForceFeedback = double(3.0)
        ...
        if ((len(constraintMatrixInline) > 0) & (useKalmanFilter == True)):
            ...
            #STEP 2: Get empirical force vector from interpolation
            #Step 2a: Get displacement of contact node
            ...
            #Step 2b: Get force magnitude and projected displacement along force vector of simulation reaction force
            ...
            #Step 2c: Get Reality-Based Reference Signal
            #empiricalForceMagnitude = getProbingForces(projectedContactPointDisplacement)
            #empiricalForceMagnitude = makeNonlinear(projectedContactPointDisplacement, reactionForceMagnitude)
            ...
            #0.10 reflects position scale of haptic device. 0.3 reflects force capacity (max 3N, while our work range is 10N).
            hapticForceMagnitude = kfForceMagnitude*scaleFactor #Scale force 
            #hapticForceMagnitude = reactionForceMagnitude*scaleFactor #Scale force to fit force capacity of haptic device

            maxHapticForce = 1.99 #2.99
        ...
        #print("Contact point displacement [mm]: " + str(projectedContactPointDisplacement) + "Haptic force [N]: " + str(kfForceMagnitude))
        #print("Contact point displacement [mm]: " + str(projectedContactPointDisplacement) + "Contact force at meniscus: " + str(reactionForceVector))
        #print("nbofs: " + str( len(self.target.position.value)))
        print('SOFA Force: ' + str(reactionForceMagnitude) + '  Interpolation: ' + str(empiricalForceMagnitude) + ' KF Force: ' + str(kfForceMagnitude) + '  Haptic Force: ' + str(hapticForceMagnitude))
        ...
        if ((len(constraintMatrixInline) > 0) & (useKalmanFilter == True)):
            ...
            #Calculate haptic force
            scaleFactor =  0.5 #This value must be tuned manually. 
            kfForceMagnitude = kfForceMagnitude*scaleFactor #Scale force to fit force capacity of haptic device

            maxHapticForce = 2.99
            if kfForceMagnitude > maxHapticForce:  #Prevent excessive force on haptic device
                kfForceMagnitude = maxHapticForce

            hapticForce = np.multiply(kfForceMagnitude,principalDirection)
            kfForceMagnitude = kfForceMagnitude * (1/scaleFactor) #Scale force back for plotting purposes
        ...
        #print("Contact point displacement [mm]: " + str(projectedContactPointDisplacement) + "Contact force at meniscus: " + str(reactionForceVector))
        #print("nbofs: " + str( len(self.target.position.value)))
        print('SOFA Force: ' + str(reactionForceMagnitude) + '  Interpolation: ' + str(empiricalForceMagnitude) + ' KF Force: ' + str(kfForceMagnitude))
        #print('Time step:   ' + str(dt) + '  Refresh rate:  ' + str(1/dt) + '[Hz]')

        #print('Reaction Force Vector: ' + str(reactionForceVector) + '  Displacement Vector of node: ' + str(contactPointDisplacement))

        if exportCSV:
            dataPackage = np.array([dt, projectedContactPointDisplacement, reactionForceMagnitude, empiricalForceMagnitude, kfForceMagnitude])
            ...
            with open("C:/Users/objel/Documents/userStudyMeniscus/USER_DATA/meniscusExaminationForces_Test3.csv", "a") as csv_file:
            ...
            dataPackage2 = np.array([reactionForceVector.flatten(), contactPointDisplacement.flatten()])
            dataPackage2 = dataPackage2.reshape(1,-1)
            with open("C:/Users/objel/Documents/MENISCUS_EXAMINATION_SIMULATOR/SOFAPYTHON3/simulationReactionForce_Test3.csv", "a") as csv_file:
